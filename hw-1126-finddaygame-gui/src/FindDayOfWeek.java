...
public class FindDayOfWeek {
	...
	// 음력 날짜에 대한 요일 반환 (신규 추가됨.)
	public static String findDayOfWeek_lun(int year, int month, int dayOfMonth) {
		KoreanLunarCalendar luncal = KoreanLunarCalendar.getInstance();
		int year_sol, month_sol, day_sol;
		
		luncal.setLunarDate(year, month, dayOfMonth, false);
		
		year_sol=luncal.getSolarYear();
		month_sol=luncal.getSolarMonth();
		day_sol = luncal.getSolarDay();
		
		return FindDayOfWeek.findDayOfWeek(year_sol,month_sol,day_sol);
	}
